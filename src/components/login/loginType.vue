    ...
    <section autocomplete="off" class="loginarea loginareaN">
        ...
        <div class="pic_warp jusernamePhone">
          <div class="label">
            <input
              ...
              @click="inputClick"
              @blur="inputFocus"
              @input="checkPhone(phoneReg)"
              type="tel"
              class="inptxt usernamePhone"
              name="userName"
              placeholder="手机号"
              id="usernamePhone"
              maxlength="11"
          ...
          <em
            class="cleanInput cleanInputR90"
            style="display: none;"
            @click="clearText"
          ></em>
          <a
            @click="getVerifyCode"
            class="btn_org getIphoneCode getPhoneCodeYes"
            id="getPhoneCode"
            href="javascript:;"
            v-html="verifyHTML"
            >获取验证码</a
      ...
      <div class="button_area " @click="login">
        ...
        <input
          ...
          type="button"
          id="SubmitBtn"
          class="btn_org SubmitBtnYes"
          value="登录"
        />
      </div>
      <div class="phoneLoginTip phoneLoginShow" v-if="isPhoneLogin">
        新会员限时惊喜 首次登录送百元礼包
      </div>
      <div class="userLocal" v-else>
        <span class="userLocalH"><a href="javascript:;">忘记密码</a></span>
      </div>
    </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    checkVertificationCode() {
      if (this.vertificationCode == (this.verifyCode || "5467")) {
