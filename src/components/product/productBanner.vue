      ...
      <div class="bd huodong huodongN">
        ...
        <ul class="pro_sku">
          ...
          <li class="proJuanSend canCoupon" style="display: block;">
            <span>{{
              isSupportCoupon == true ? "可用优惠券" : "不可以优惠券"
            }}</span>
          </li>
        </ul>
      </div>
...
export default {
  ...
  computed: {
    ...
    taber() {
      return (
        Array.isArray(this.banners) &&
        this.banners.map((value, index) => {
  ...
  methods: {
    ...
    getBanners() {
      request({
        url: "/product/getCommodityDetail.sc?cNo=" + this.productId
      }).then(res => {
